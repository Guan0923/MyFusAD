...
class FusAD_layer(L.LightningModule):
    ...
    def __init__(
        ...
        self.asm = Adaptive_Spectral_Module(
            dim, drop=drop, adaptive_filter=adaptive_filter
        )
        self.drop_path = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.ifm = IFM(in_features=dim, hidden_features=mlp_hidden_dim, kernel_size=ifm_kernel_size, drop=drop)
...
class FusAD(nn.Module):
        ...
        self.jafa_blocks = nn.ModuleList(
            [
                FusAD_layer(
                    ...
                    ifm_kernel_size=args.ifm_kernel_size,
                )
    ...
    def pretrain(self, x_in):
        ...
        xb_mask, _, self.mask, _ = random_masking_3D(
            x_patched, mask_ratio=self.args.mask_ratio
        )
...
class model_training(L.LightningModule):
    ...
    def _calculate_loss(self, batch, mode="train"):
        ...
        self.log(
            f"{mode}_mse", mse, on_step=False, on_epoch=True, prog_bar=True, logger=True
        )
        self.log(
            f"{mode}_mae", mae, on_step=False, on_epoch=True, prog_bar=True, logger=True
        )
...
def train_model(
    ...
    if args.load_from_pretrained and pretrained_model_path:
        model = model_training.load_from_checkpoint(
            pretrained_model_path, args=args, weights_only=False
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ifm_kernel_size", type=int, default=5, help="kernel size for IFM module")
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="False: without pretraining",
    )
    ...
    parser.add_argument(
        "--adaptive_filter", type=str2bool, default=True, help="使用自适应滤波"
    )
    ...
    run_description += f"_sl{args.seq_len}_pl{args.pred_len}_bs{args.batch_size}_mr{args.mask_ratio}"
    run_description += f"_seed{args.seed}_dp{args.dropout}"
    run_description += f"_preE{args.pretrain_epochs}_ftE{args.train_epochs}"
    run_description += f"_IFMks{args.ifm_kernel_size}"
    ...
    df.to_excel(
        os.path.join(
            CHECKPOINT_PATH, f"results_{datetime.datetime.now().strftime('%H_%M')}.xlsx"
        )
    )
